<template>
  <div v-loading="loading2" element-loading-text="拼命加载中"
    ...
    <div>
      ...
      <span class="btnBox">
        <el-button size="small" @click="toWorkplace(scope)">接诊</el-button>
      </span>
      </template>
    </el-table-column>
  </el-table>
...
export default{
  ...
  methods: {
    ...
    search () {
      ...
      searchPatient.containedIn('doctor', [this.searchInput])
    ...
    toWorkplace (scope) {
      ...
      this.$router.push({path: 'workPlace', query: {patientId: this.patientId}})
    }
  }
}
</script>
